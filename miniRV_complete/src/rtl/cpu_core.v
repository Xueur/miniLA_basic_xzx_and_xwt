...
`endif
    ...
    wire mem_done = mem_is_load ? (mem_req_sent && daccess_rvalid) :
                    mem_is_store ? (mem_req_sent && daccess_wresp) : 1'b1;
    wire mem_wait = mem_is_access && !mem_done;

    wire mem_resp_active = daccess_rvalid || daccess_wresp;
    assign daccess_ren   = mem_is_load && !mem_req_sent &&
                           !mem_resp_active ? mem_da_ren : 4'h0;
    assign daccess_wen   = mem_is_store && !mem_req_sent &&
                           !mem_resp_active ? mem_da_wen : 4'h0;
