...
set project_dir [file join $script_dir vivado_c_test_board_50MHz]

create_project -force miniRV_c_test_board_50MHz $project_dir \
...
if {![file exists $active_image]} {
    error "Missing software/active_test/active_test.hex. Run tools/select_c_test_image.py first."
}
set image_fp [open $active_image r]
...
add_files -norecurse -fileset sources_1 $active_image
set_property file_type {Memory Initialization Files} \
    [get_files $active_image]

set active_banks [list]
...
for {set bank 0} {$bank < 5} {incr bank} {
    ...
    if {![file exists $bank_file]} {
        error "Missing $bank_file. Run tools/select_c_test_image.py first."
    }
    ...
    if {[llength $words] != 8192} {
        error "BRAM bank $bank must contain 8192 words."
    }
    set bank_word_list [concat $bank_word_list $words]
    lappend active_banks $bank_file
}
if {[join $bank_word_list "\n"] ne [join $image_word_list "\n"]} {
    error "Active C_TEST BRAM banks do not reconstruct active_test.hex."
}
add_files -norecurse -fileset sources_1 $active_banks
set_property file_type {Memory Initialization Files} \
    [get_files $active_banks]
...
set_property generic {
    MEM_BANK0_FILE="active_test_bank0.hex"
    MEM_BANK1_FILE="active_test_bank1.hex"
    MEM_BANK2_FILE="active_test_bank2.hex"
    MEM_BANK3_FILE="active_test_bank3.hex"
    MEM_BANK4_FILE="active_test_bank4.hex"
} \
...
puts "Created [file join $project_dir miniRV_c_test_board_50MHz.xpr]"
puts "SoC clock: 50 MHz (pipelined SoC requirement satisfied)"
puts "Program image: $active_image (5 x 8192-word BRAM banks / 160 KiB)"
puts "C_TEST UART: 115200-8-N-1"
