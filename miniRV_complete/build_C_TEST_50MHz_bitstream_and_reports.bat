...
set "XPR=%~dp0vivado_c_test_board_50MHz\miniRV_c_test_board_50MHz.xpr"
if not exist "%XPR%" (
    call "%VIVADO_CMD%" -mode batch -notrace -source create_c_test_board_project.tcl
    if errorlevel 1 (
...
call "%VIVADO_CMD%" -mode batch -notrace -source build_c_test_board_bitstream.tcl
if errorlevel 1 (
    echo Build or timing validation failed.
    echo Check reports\c_test_board_50MHz and the messages above.
    pause
    exit /b 1
)

echo.
echo C_TEST 50 MHz timing passed. The bitstream and reports are ready.
echo Bitstream: output\miniRV_c_test_active_50MHz.bit
echo Timing:   reports\c_test_board_50MHz\timing_summary_PASS.rpt
echo.
start "" "%XPR%"
pause
exit /b 0
